...
def word_span_means(data: np.ndarray, start_centers: np.ndarray, stop_centers: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Mean activity over each word's own [start, stop) sample span.

    Unlike ``segment_means`` (a fixed offset window around a single center),
    this integrates each channel over the variable-length span of the present
    word. ``start_centers`` and ``stop_centers`` are per-word sample indices.
    Words shorter than one sample are widened to a single sample so that any
    word with a valid onset still yields a feature.
    """
    n_channels, n_times = data.shape
    starts = np.asarray(start_centers, dtype=np.int64)
    stops = np.maximum(np.asarray(stop_centers, dtype=np.int64), starts + 1)
    valid = (starts >= 0) & (stops <= n_times) & (stops > starts)
    means = np.full((len(starts), n_channels), np.nan, dtype=np.float32)
    if valid.any():
        csum = np.concatenate(
            [np.zeros((n_channels, 1), dtype=np.float64), np.cumsum(data.astype(np.float64), axis=1)],
            axis=1,
        )
        seg_sum = csum[:, stops[valid]] - csum[:, starts[valid]]
        means[valid] = (seg_sum / (stops[valid] - starts[valid])[None, :]).T.astype(np.float32)
...
def _cosine_normalize_rows(v: np.ndarray) -> np.ndarray:
    v = v - v.mean(axis=1, keepdims=True)
    return v / (np.linalg.norm(v, axis=1, keepdims=True) + 1e-12)


def identification_rank_per_word(y_true: np.ndarray, y_pred: np.ndarray) -> np.ndarray:
    """Per-item rank-identification score in [0, 1] (chance = 0.5).

    Item i scores the fraction of other items whose true vector is less similar
    to pred_i than the true target is.
    """
    finite = np.isfinite(y_true).all(axis=1) & np.isfinite(y_pred).all(axis=1)
    out = np.full(len(y_true), np.nan, dtype=np.float32)
    if not finite.any():
        return out
    yt = _cosine_normalize_rows(y_true[finite].astype(np.float32))
    yp = _cosine_normalize_rows(y_pred[finite].astype(np.float32))
    n = len(yt)
    if n < 2:
        return out
    sims = yp @ yt.T
    true_sim = np.diag(sims)
    beaten = (sims < true_sim[:, None]).sum(axis=1)
    out[finite] = (beaten / (n - 1)).astype(np.float32)
    return out


def word_metric_scores_from_npz(npz, metric_suffix: str) -> np.ndarray:
    """Per-word scores for analysis: point_r or rank-identification fraction."""
    if metric_suffix == "point_r":
        return np.asarray(npz["point_r"], dtype=np.float32)
    if metric_suffix == "rank_identification":
        return identification_rank_per_word(np.asarray(npz["y_true"]), np.asarray(npz["y_pred"]))
    raise ValueError(f"Unknown metric: {metric_suffix!r}")


def identification_rank_accuracy(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """Mean rank-identification accuracy (chance = 0.5).

    For each held-out item i, compare the cosine similarity between its predicted
    vector and its true vector against the similarity to every other item's true
    vector (the distractors). The score for item i is the fraction of distractors
    the true target beats; the metric is the mean across items. This is the
    all-distractors version of pairwise "pick the true word over a distractor"
    accuracy, and is far more sensitive than per-word correlation.
    """
    per_word = identification_rank_per_word(y_true, y_pred)
    if not np.isfinite(per_word).any():
        return float("nan")
    return float(np.nanmean(per_word))
