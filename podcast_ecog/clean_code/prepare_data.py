...
decoding scripts into one local cache. It reads the mounted BIDS dataset, but
all outputs are written under ``podcast_ecog/outputs/clean_code`` by default.
"""
...
def prepare_subject_neural(args: argparse.Namespace, words: pd.DataFrame, subject: str) -> None:
    ...
    if npz_path.is_file() and channel_path.is_file() and not args.force:
    ...
    if args.picks_regex:
        ...
        if len(picks) == 0:
            raise ValueError(f"{sub}: no channels match {args.picks_regex!r}")
        raw.pick(picks)
    ...
    x, valid = segment_means(data, centers, float(raw.info["sfreq"]), args.window_start, args.window_end)
